...
contract ERA_v1 is EthereumRegistrationAuthorityInterface, Ownable {
    ...
    modifier onlyDomainOwner(uint256 _domainHash) {
        address tempOwner = require(domainOwnerMap[_domainHash];
        if (tempOwner == 0) {
    ...
    function addDomain1(uint256 _domainHash, address _domainAuthority, address _orgInfo, address _domainOwner) external onlyOwner {
        // Check that the entry does not exist before adding it.
        require(authorityMap[_domainHash] == address(0) && orgInfoMap[_domainHash] == address(0) && domainOwnerMap[_domainHash] == address(0));
        emit DomainAdded(_domainHash);

        if (_domainAuthority != address(0)) {
            authorityMap[_domainHash] = _domainAuthority;
        }
        if (_orgInfo != address(0)) {
            orgInfoMap[_domainHash] = _orgInfo;
        }
        if (_domainOwner != address(0)) {
